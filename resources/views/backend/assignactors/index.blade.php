...
<h3 class="text-dark d-inline-block">AssignActor List</h3>
<span><a href="{{route('assignactors.create')}}" class="btn btn-info" style="position: absolute; right: 10px;">Create</a></span>
    </div>
  ...
  <tbody>
    ...
    <tr>
      ...
      <td>{{$movie->name}}</td>
       @endforeach --}}
      <td>
        <a href="{{route('assignactors.edit',$info->id)}}" class="btn btn-outline-info "><i class="fas fa-pencil-alt"></i></a>
       <form method="post" action="{{route('assignactors.destroy',$info->id)}}" class="d-inline-block" id="delete-info{{ $info->id }}">
          @csrf
          @method('delete')
